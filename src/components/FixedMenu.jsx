...
const FixedMenu = () => {
  ...
  return (
    <div className="fixed w-full h-[400px] z-50 flex justify-center pointer-events-none bg-transparent ">
      {/* menu */}
      <AnimatePresence>
        {console.log("showMenu:", showMenu, "showMenuButton:", showMenuButton)}
        {showMenu && showMenuButton && (
          <motion.div className="relative w-full max-w-md md:max-w-none h-[400px] bottom-[28rem] xl:bottom-[21.2rem] px-4 pointer-events-auto">
            <div className="bg-white w-full h-full max-w-[1170px] mx-auto py-12 xl:py-12 px-12 xl:px-32 flex items-center gap-12">
              menu
            </div>
          </motion.div>
        )}
      ...
      {isMobile ? (
        <div className="fixed z-50 bottom-16">
          <button
            ...
            className="bg-p-4 w-[54px] h-[54px] rounded-lg cursor-pointer flex items-center justify-center select-none pointer-events-auto"
          >
            <CgMenuGridR className="text-4xl text-n-1" />
          </button>
        </div>
      ) : (
        <AnimatePresence>
          {showMenuButton && (
            <motion.div
              ...
              className="fixed z-50 bottom-16 pointer-events-auto will-change-transform"
            >
              <button
                onClick={() => setShowMenu(!showMenu)}
                className="bg-p-4 w-[54px] h-[54px] rounded-lg cursor-pointer flex items-center justify-center select-none"
              >
                <CgMenuGridR className="text-4xl text-n-1" />
              </button>
            </motion.div>
